    ...
    onAuthStateChanged(auth, (user) => {
        ...
        function filtrarEAtualizarInterface() {
            ...
            bancoDados.forEach((item, index) => {
                ...
                if (item.conta === contaSelecionada && mesItem === mesSelecionado && anoItem === anoSelecionado) {
                    contagemItens++;
                    somatorioValores += parseFloat(item.valor);

                    const dataBR = item.data.split('-').reverse().join('/');
                    const valorBR = parseFloat(item.valor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
                    ...
                    linha.innerHTML = `
                        ...
                        <td>${dataBR}</td>
                        <td>
                            <span style="padding: 5px 10px; border-radius: 6px; font-weight: bold; color: white; background: ${item.status === 'Pago' ? '#22c55e' : '#ef4444'};">
